var PatientView = {
    ...
    create: function(pidParam, docCount) {
        ...
        var callback = function (response) {
            ...
            rcol += '</tbody></table>';            
            //only show documents link if docs exist, else show message
            if (docCount > 0) {
                rcol += '<div class="link"><a href="#/documents/' + pidParam + '">Documents (' + docCount + ')</a></div>';
            } else {
                rcol += '<div class="link"><p>No digitized copies available at this time, but original copies maintained at the Beck Archives.</p><br><a href="#/dashboard/updateRecord/' + pidParam + '">Edit Record</a></div>';
            }
